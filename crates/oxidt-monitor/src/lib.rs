...
//!
//! ```no_run
//! use oxidt_monitor::{HealthCheck, Kpi, MonitorSource, MonitorToken};
//!
...
//!     .merge(oxidt_monitor::router(Metrics(state), MonitorToken::from_env()))
//! # }
//! ```
...
///
/// Implemented by the host app over its `AppState`, the same way `oxidt-auth`'s
/// stores are.
#[async_trait::async_trait]
